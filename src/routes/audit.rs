...
pub async fn list(State(state): State<AppState>, jar: CookieJar, headers: HeaderMap) -> Result<Response, Redirect> {
    ...
    let logs = sqlx::query_as::<_, AuditLog>("SELECT * FROM audit_logs ORDER BY created_at DESC LIMIT 200")
        .fetch_all(&state.pool).await.unwrap_or_default();

    if is_htmx(&headers) && !is_nav_request(&headers) {
        Ok(Html(AuditListFragment { logs }.render().unwrap_or_default()).into_response())
    } else {
        Ok(Html(AuditTemplate { active_nav: "audit", logs }.render().unwrap_or_default()).into_response())
    }
}
